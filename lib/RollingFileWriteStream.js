...
class RollingFileWriteStream extends Writable {
  ...
  _moveOldFiles(cb) {
    ...
    this._getExistingFiles(async (e, files) => {
      try {
        const filesToMove = [];
        ...
        const moveAndMaybeCompressFileAsync = promisify(moveAndMaybeCompressFile);
        for (let idx = 0; idx < filesToMove.length; idx++) {
          const files = filesToMove[ idx ];
          debug(
            `src=${files.sourceFilePath}, tgt=${
              files.sourceFilePath
            }, idx=${idx}, pos=${filesToMove.length - 1 - idx}`
          );
          await moveAndMaybeCompressFileAsync(
            files.sourceFilePath,
  ...
  _deleteFiles(fileNames, done) {
    ...
    Promise.all(fileNames.map(f => fs.unlink(path.format({ dir: this.fileObject.dir, base: f }))))
